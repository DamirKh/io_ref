...
def write_as_python():
    ...
    # ic(project_chass)
    ms = f"""## Created {datetime.datetime.now().isoformat()}"""
    """## autogenerated by IO Table Generator"""
    for CHASSI in project_chass:
        cn = f'CHASSIS {CHASSI}'
        ms += f"""
## {cn:=^22}
...
## └───────────────────┘"""
            for CHANNEL in io_config[CHASSI][SLOT].keys():
                ...
                descr = ' '.join(io_description.get(CHASSI, {}).get(SLOT, {}).get(CHANNEL, '').split())
                ms += f"""\n{tag} = '{base_tag}'  #{descr}"""

    return ms


def write_io_map_yaml():
    def format_io_entry(tag: str, data: dict) -> str:
        """Форматирует одну запись в одну строку с кавычками"""
        if "bit" in data:
            return f'{tag}: {{ basetag: "{data["basetag"]}", bit: {data["bit"]}, description: "{data["description"]}" }}'
        else:
            return f'{tag}: {{ basetag: "{data["basetag"]}", description: "{data["description"]}" }}'

    global io_config, io_description, io_base
    project_chass = sorted(io_config.keys())
    ms = io.StringIO()
    # Заголовок
    ms.write(f"## autogenerated file\n")
    ms.write(f"## {datetime.datetime.now().isoformat()}\n")
    ms.write("## changes will be overwritten!\n\n")

    for CHASSI in project_chass:
        cn = f'CHASSIS {CHASSI}'
        ms.write( f"""\n\n## {cn:=^22}\n\n""" )
        slot_list = sorted(io_config[CHASSI].keys())
        for SLOT in slot_list:
            ms.write(f"""\n## SLOT {SLOT:02}\n""")
            io_map = {}
            for CHANNEL in io_config[CHASSI][SLOT].keys():
                tag = io_config[CHASSI][SLOT][CHANNEL]
                base_with_chan = io_base[CHASSI][SLOT][CHANNEL]
                # "".rsplit('.', 1)[0]
                # "".isdigit()
                base_no_chan, chan = base_with_chan.rsplit('.', 1)
                if chan.isdigit(): # discrete input/output
                    ch = int(chan)
                    io_map[tag] = {
                        "basetag": base_no_chan,
                        "bit": ch,
                        "description": ' '.join(io_description.get(CHASSI, {}).get(SLOT, {}).get(CHANNEL, '').split())
                    }
                else:  # real input/output
                    io_map[tag] = {
                        "basetag": base_with_chan,
                        "description": ' '.join(io_description.get(CHASSI, {}).get(SLOT, {}).get(CHANNEL, '').split())
                    }
            for tag, data in io_map.items():
                ms.write(format_io_entry(tag, data) + "\n")

    return ms.getvalue()


def write_io_map_lua():
    def format_lua_entry(tag: str, data: dict) -> str:
        """Форматирует одну запись в Lua-таблицу"""
        # Если тег содержит спецсимволы (/,-,), используем квадратные скобки
        needs_brackets = any(c in tag for c in '/-(). ')
        if needs_brackets:
            tag_key = f'["{tag}"]'
        else:
            tag_key = tag
        ...
        if "bit" in data:
            return f'    {tag_key} = {{ tag = "{data["basetag"]}", bit = {data["bit"]}, description = "{data["description"]}" }},'
        else:
            return f'    {tag_key} = {{ tag = "{data["basetag"]}", description = "{data["description"]}" }},'
    ...
    ms.write(f"-- {datetime.datetime.now().isoformat()}\n")
    ms.write("-- changes will be overwritten!\n")
    ms.write("-- usage: local tags = require(\"config.this_file\")\n\n")
    ms.write("return {\n")
    ...
    for CHASSI in project_chass:
        ...
        for SLOT in slot_list:
            ...
            for tag, data in io_map.items():
                ms.write(format_lua_entry(tag, data) + "\n")
